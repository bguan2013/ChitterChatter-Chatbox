...
//----------------------------------------------------------------------------------------------        
        ...
        if (n == 1) {
            ...
            address = InetAddress.getByName("wolf.cs.oswego.edu"); // moxie's address
            multicastAddress = InetAddress.getByName("224.6.7.8");
        } else {
            address = InetAddress.getByName("pi.cs.oswego.edu"); // moxie's address
...
//        multicastAddress = InetAddress.getByName("224.7.8.9");
        ...
        // should only join multicast if successgful join
//        multiSocket.joinGroup(multicastAddress);
//        if (multiSocket.isConnected()) {
//            System.out.println("MULTICAST CONNECTION");
//        } else {
...
//        }
        ...
        // if you dont recieve a packet, the socket will timeout
...
//------------------------------------------------------------------------------
                            if (codeNum == (short) 01) {
                            ...
                            } else if (codeNum == (short) 2) { //no username
                                // leave case  - print message saying username left
                                System.out.println("DEBUG: received a leaving packet");
                                ...
                                System.out.println("DEBUG: Server sends me last 30 messages ");
                                ...
                                // vote result
                                System.out.println("DEBUG: result of a vote");
                                addUserMessage(check, temp.getLength());

                            } else if (codeNum == (short) 10) {
                                // ping acknoledged
                                System.out.println("DEBUG: Ping acknowledged");

                            } else if (codeNum == (short) 12) {
                                // received a specific message from a specific username

                            } else if (codeNum == (short) 14) {
                                // receive latest packet
                                System.out.println("DEBUG: received the latest packet");
                                addUserMessage(check, temp.getLength());

                            } else if (codeNum == (short) 15) {
                                // join session DENIED
                                System.out.println("DEBUG: server denied join request");
                                addMessage(check, temp.getLength());

                            } else if (codeNum == (short) 16) {
                                // judging a voting session
                                // System.out.println("DEBUG: Voting decisions are being made");
                                addMessage(check, temp.getLength());
                                ...
                                // send username to the GUI
                                System.out.println("DEBUG: Sending usernames to GUI");
...
//------------------------------------------------------------------------------
                            if (codeNum == (short) 01) {
                                // generic message    
                                // print the new text on the client's screen
                                System.out.println("DEBUG: RECEIVED A MULTICAST PACKET");
//                                System.out.println("PACKET LENGTH: " + temp.getLength());
//                                System.out.println("PACKET LENGTH: " + temp.getData().length);
                                addUserMessage(check, temp.getLength());
...
//------------------------------------------------------------------------------                        
            ...
            try {
                ...
                if (!(tempAck.equals(nullCheck))) {
                    printMessage("DEBUG: Joined " + address.getHostName() + " group");
...
// receives a list of the last 30 messages in the chat history    
    public void getChat() {
                ...
                if (server1 != null && server2 != null) {
                    ...
                    try {
                        ...
                        if (getStatus() == false) {
                            ...
                            Thread.sleep(10000);
                            if (getStatus() == false) {
                                gui.displayMsg("System", "Couldnt connect to any other server, client is going to close");
                                try {
                                    Thread.sleep(20000);
