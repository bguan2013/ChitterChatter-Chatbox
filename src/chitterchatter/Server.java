...
public class Server {
    ...
    static final int sendPort = 2718;
    ...
    public static void main(String[] args) throws SocketException, IOException, ClassNotFoundException, InterruptedException {
        String usage = "usage: type server name (moxie | wolf | pi)";
        ...
        if (args.length != 1 || (!args[0].equalsIgnoreCase("moxie")
            ...
            sh = new ServerHandler(args[0]);

            ssock1 = new DatagramSocket();
            ssock2 = new DatagramSocket();
            // 12 for username, 4 for space, 1024 for message size
//            Pmraf raf = new Pmraf(rafBlockSize);
//            PArray parr = new PArray(numOfStoredMessage, 30, raf);
        }
...
//------------------------------------------------------------------------------
            if (codeNum == (short) 1) {
                ...
                System.out.println("DEBUG: recived a message from client");
                sh.addMessage(check);
                ...
                System.out.println("DEBUG: recieved a leaving packet");
                sh.leavingChat(check, (short) 1);
                //leaveMessage(check, temp.getAddress());
            ...
            // join case    
                ...
                if (temp.getAddress() != server1 && temp.getAddress() != server2) {
                    ...
                    if (bool == true) {
                        System.out.println("DEBUG: successful join");
                        ...
                        //successfulJoinMessage(check, temp.getAddress());
                    } else {
                        System.out.println("DEBUG: unsuccessful join");
                        unsuccessfulJoinMessage(temp.getAddress(), temp.getPort());
                    }
                }
            } else if (codeNum == (short) 4) {
                // print request case    
                System.out.println("DEBUG: send all messages to a user");
                ...
                System.out.println("DEBUG: Begin a vote");
                ...
                System.out.println("DEBUG: a vote packet");
                short vote;
                byte[] v = new byte[2];
                ...
                if (flagCode == (short) 1) {
                    ...
                    System.out.println("DEBUG - SL: " + username + " was removed");
                    ...
                    System.out.println("DEBUG - SL:" + username + " was added");
                ...
                if (code == (short) 0) {
                    System.out.println("DEBUG - SL: vote was successful for " + victimName);
                ...
                System.out.println("DEBUG: Ping test");
                sendPingAck(temp.getAddress(), temp.getPort());
            } else if (codeNum == (short) 11) {
                // requesting a specific packet
                sendPacketbyNumber(check, temp.getAddress(), temp.getPort());
            } else if (codeNum == (short) 13) {
                // get latest packet
                System.out.println("DEBUG: send the latest packet");
                String[] latest = sh.getLatest();
                sendLatest(latest, temp.getAddress(), temp.getPort());
                ...
                System.out.println("DEBUG: geting votes from non judge");
                ...
                System.out.println("DEBUG: send usernames");
                sendUsers(temp.getAddress(), temp.getPort());
            } else if (codeNum == (short) 22) {
                System.out.println("DEBUG: send usernames for gui");
                sendUsersForGuiKick(temp.getAddress(), temp.getPort());
            } else if (codeNum == (short) 24) {
                System.out.println("DEBUG: send all usernames for server");
                sendUsersForServer(temp.getAddress());
            } else if (codeNum == (short) 25) {
                System.out.println("DEBUG: recieved all usernames from server");
                ...
                System.out.println("DEBUG: Send sent to other server");
                if (sh.parr.size() > 0) {
                    sendChatToServer(temp.getAddress());
                }
            } else if (codeNum == (short) 29) {
                System.out.println("DEBUG: recieved request to update");
                otherChatMethod(check, temp.getLength());
            } else if (codeNum == (short) 30) {
                System.out.println("DEBUG: Update chat of server");
...
//------------------------------------------------------------------------------            
    ...
    static void sendMessage(final byte[] b, final int packetSize, final InetAddress source) {
        Thread t = new Thread("sendMessage") {
            ...
            public void run() {
                ...
                username = new String(tempUser);
                System.out.println("DEBUG: username: " + username + " of length " + username.length());
                System.out.println("DEBUG: message: " + message + " of length " + message.length());
                ...
                try {
                    System.out.println("DEBUG: Sending message to everyone in multicast");
                    multi.send(messpack.datapacket);
                    if (!(source.equals(server1) || source.equals(server2))) {
                        System.out.println("DEBUG: sending message to other servers");
        ...
        t = new Thread("waitAndCountVotes") {
            ...
            public void run() {
                ...
                try {
                    ...
                    Thread.sleep(25000);
                    System.out.println("DEBUG: sending vote results to judge at " + add.getCanonicalHostName());
                    System.out.println("DEBUG: Yes Votes " + sh.getYVotes());
                    System.out.println("DEBUG: No Votes " + sh.getNVotes());
    ...
    static public void sendPacketbyNumber(final byte[] packet, final InetAddress add, final int port) {

        Thread t = new Thread("sendPacketbyNumber") {
            @Override
            public void run() {
                byte[] b = new byte[2];
                System.arraycopy(packet, 2, b, 0, 2);
                short s = getCode(b);
                int location = (int) s;
                String[] mess = sh.getMessageByNumber(location);
                ccpacket req = new ccpacket().sendRequest(s, mess, add, port);

                try {
                    sock.send(req.datapacket);
                } catch (IOException ex) {
                    Logger.getLogger(Server.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public static void updateUsersFromServer() {
        ...
        t = new Thread("updateUsersFromServer") {
            ...
            public void run() {
                ...
                try {
                    System.out.println("DEBUG: sending request to other Server");
                    ssock1.send(update1.datapacket);
                    Thread.sleep(10000);
        ...
        Thread t = new Thread("updateChat") {
            ...
            public void run() {
                ...
                try {
                    ...
                    System.out.println("DEBUG: Sending update request to get chat - at " + server1.getCanonicalHostName());
                    ssock1.send(update1.datapacket);
                    Thread.sleep(3000);
                    String[][] check = sh.getAllMessages();
                    if (otherChat.length > check.length) {

                        System.out.println("DEBUG: Updating the chat");
                        for (int i = 0; i < otherChat[0].length; i++) {
                        ...
                        System.out.println("DEBUG: Sending update request to get chat - at " + server2.getCanonicalHostName());
                        ssock2.send(update2.datapacket);
                        Thread.sleep(3000);
                        ...
                        if (otherChat.length > check.length) {
                            ...
                            System.out.println("DEBUG: Couldn't Update messages");
                        }
                    }
    ...
    static void sendChatToServer(final InetAddress add) {
        ...
        Thread t = new Thread("sendChatToServer") {
            @Override
            public void run() {
                System.out.println("DEBUG: sending chat request to server " + add.getCanonicalHostName() + " with message size of " + ServerHandler.parr.size());
...
//        byte[] tempNumber = new byte[2];
//        System.arraycopy(b, 2, tempNumber, 0, 2);
//        short tempNumberofMessages = getCode(tempNumber);
        System.out.println("DEBUG: adding updated messages to the new server");
...
//        short tempNumberofMessages = getCode(tempNumber);
        ...
        System.out.println("DEBUG: adding values to otherChat to compare later of " + tempNumberofMessages + " messages");
        for (int i = 0; i < tempNumberofMessages; i++) {
            byte tempByte[] = new byte[512 + 12];
            System.arraycopy(b, 4 + ((i * 12) + (i * 512)), tempByte, 0, tempByte.length);
            byte[] tempUsername = new byte[usernameLength];
