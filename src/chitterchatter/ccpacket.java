...
public class ccpacket {
    ...
    protected final short requestCode = 11;
    protected final short requestSendCode = 12;
    protected final short latestCode = 13;
    ...
    // 2 bytes per character makes the message length (512 character length)
    public final int messageLength = 512;

    // 2 bytes per character make the usernameLength 24 bytes
    ...
    // message has username added to the 
    ...
    // this packet is differnet since this only sends the username and not the message to everyone
    ...
    // client sends a message to server, that they are leaving chat
    public ccpacket leaveChat(String username, InetAddress add, int port) {

        ccpacket leave = new ccpacket();
        // quit strings are added to the username and then is sent to the server
        String quit1 = "***";
    ...
    // Client tries to join chat with their username
    ...
    // this packet tells the other servers to add or remove users from their list
    // remove flag is 0
    // add flag is 1
    ...
    // print packet is different since the server needs to know which address
    // and port number to send the previous messages to, so a different  
    // DatagramPacket is used.
    ...
    public ccpacket printPacket(String[][] messages, InetAddress add, int port) {
        ...
        // ((12 + 512) * messages[0].length) 
        // --- 12   for username
        // --- 512 for message
        // --- messages[0].length for the number of messages
        ...
        for (int i = 0; i < messages[0].length; i++) {
            ...
            // messages[i][1].getBytes().length should equal to 512 since the PArray.getAllMessage() should fill the strings with " " 's 
            // to meet the 512 length
    ...
    // pack send by teh client to start a vote to kick someone from chat
    ...
    // packet from client to vote yes or no to voting someone out
    ...
    // message to all clients stating that a user was voted out of chat
    ...
    // ACK packet from a ping for the client/other servers
    ...
    // packet sent from client to ask for a specific message
    public ccpacket requestPacket(short packetNum, InetAddress add, int port) {
        ccpacket req = new ccpacket();

        req.data = new byte[4];
        req.data[0] = (byte) (requestCode & 0xff);
        req.data[1] = (byte) ((requestCode >> 8) & 0xff);
        req.data[2] = (byte) (packetNum & 0xff);
        req.data[3] = (byte) ((packetNum >> 8) & 0xff);

        req.datapacket = new DatagramPacket(req.data, req.data.length, add, port);

        return req;
    }

    // packet sent from server with the clients requested packet
    public ccpacket sendRequest(short packetNum, String[] mess, InetAddress add, int port) {

        ccpacket req = new ccpacket();

        req.data = new byte[4 + mess[1].getBytes().length + usernameLength];
        req.data[0] = (byte) (requestSendCode & 0xff);
        req.data[1] = (byte) ((requestSendCode >> 8) & 0xff);
        req.data[2] = (byte) (packetNum & 0xff);
        req.data[3] = (byte) ((packetNum >> 8) & 0xff);
        System.arraycopy(mess[0].getBytes(), 0, req.data, 4, usernameLength);
        System.arraycopy(mess[1].getBytes(), 0, req.data, 4 + usernameLength, mess[0].getBytes().length);
        req.datapacket = new DatagramPacket(req.data, req.data.length, add, port);

        return req;
    }

    // packet to request the most recent packet from the server
    public ccpacket getLastest(InetAddress add, int port) {
        ccpacket late = new ccpacket();

        late.data = new byte[2];
        late.data[0] = (byte) (latestCode & 0xff);
        late.data[1] = (byte) ((latestCode >> 8) & 0xff);
    ...
    // packet from server to client of the most recent message
    ...
    // packet is sent to all other non-Judge servers when a vote is being started
    ...
    // packet used to count up all the votes from the other servers 
